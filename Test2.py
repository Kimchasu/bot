                ...
                if (start != -1 and end != -1) and start < end:  # [] 조건 찾기. [, ]가 존재해야 하고, 닫는 괄호가 여는 괄호보다 앞에 있으면 안됨
                    ...
                    if first_keyword in keywords:  # 준비된 키워드 내용 중 키워드가 있다면
                        ...
                        if first_keyword == '주사위':  # 첫번째 키워드가 '주사위' 라면
                            ...
                            print('! 노래방점수를 확인합니다.')
                            keyword_action_return = str(Song2(mention))
                            keyword_type = 8
                            ...
                            if item == '운세종이' :
                                ...
                                luckysentence = '\n\n'+'<'+lucky.format(color=random.choice(colorli),direction=random.choice(directionli),woter=random.choice(woterli),Lucky=random.choice(Lukli),drink=random.choice(drinkli))+'>'
    ...
    for i in range(len(Shop_List)):
        if (Shop_List[i]['2']) == item:
            ...
            Sentence = Shop_List[i]['3'].format(condom=random.choice(condomlist), sleep=random.choice(sleeplist),
                                                earmuffs=random.choice(earmuffslist))
            print(Sentence)
...
def Song2(score) :
    ...
    if 0 < score <= 20 :
        Evaluation = str(score) + "마이크테스크 원"
    elif 20 < score <= 40 :
        Evaluation = str(score) + "마이크테스크 투"
    elif 40 < score <= 60 :
        Evaluation = str(score) + "마이크테스크 쓰리"
    elif 60 < score <= 80 :
        Evaluation = str(score) + "마이크테스크 포"
    elif 80 < score <= 100 :
        Evaluation = str(score) + "마이크테스크 파이브"
...
def make_reply_content(mention, type_of_keyword, keyword_action_return):
    ...
    try:
        if type_of_keyword == 1:  # 키워드 타입이 출석일 경우
        ...
        elif type_of_keyword == 8:  # 노래방점수 Test
            Ev = keyword_action_return
            cell = worksheetco.find(mention.author.screen_name)
            print("%s행 %s열에서 찾았습니다." % (cell.row, cell.col))
            ...
            reply_content = "아아. " + str(name) + "마이크테스트원." + "\n\n" + "두구두구두구......" + str(Ev)
